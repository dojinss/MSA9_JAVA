...
public class StreamArrayList {
	...
	public static void main(String[] args) {
		...
		personList.add( new Person( "박조은", 30));
		personList.add( new Person( "황조은", 22));
		personList.add( new Person( "조은", 27));
		personList.add( new Person( "심조은", 41));
		personList.add( new Person( "강조", 35));
		personList.add( new Person( "윤조", 20));
		
		// map - 매핑
		// : 스트림 요소를 사용하려는 형태로 반환하는 연산
		...
		personList.stream()
				  .filter( person -> person.getName().length() > 2)
				  .forEach( p -> System.out.println(p));
		System.out.println();
...
//				  ;
		...
		// 이름이 세글자인 사람의 수 - 요소의 수 count()
		System.out.println("::::: 이름 세글자인 사람의 수 :::::");
		long count = personList.stream()
							   .filter( p -> p.getName().length() > 2 )
							   .count();
		System.out.println("이름이 세글자인 사람들의 수 : " + count);
		
		// sum() - 스트림 요소의 합계
		// 이름이 두글자인 사람들 나이의 함계
		System.out.println("::::: 이름이 두글자인 사람들의 나이합 :::::");
		int sum = personList.stream()
							.filter( p -> p.getName().length() == 2)
							.mapToInt( p -> p.getAge() )
							.sum();
		System.out.println("이름이 두글자인 사람들 나이의 합계 : " + sum);
